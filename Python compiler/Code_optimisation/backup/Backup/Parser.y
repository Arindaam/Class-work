...
elif_stmts :T_Elif{scope=1;loop_init();} bool_exp{loop_condition();} T_Cln T_NL T_id final suite{endblock();} elif_stmts |T_Else{scope=1;loop_init();} T_Cln T_NL T_id final suite{printf("L%d",labels[ltop--]);} try|try;
...
while_stmt : T_While {pops=label_n++;loop_init();}bool_exp{loop_condition();} T_Cln T_NL T_id new nsuite{printf("Goto L%d",pops);printf("L%d:",label_n-1);}; 
